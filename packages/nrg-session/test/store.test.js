...
describe('test/store.test.js', () => {
  ...
  describe('defer session middleware', () => {
    ...
    it('should handler session error when store.set error', (done) => {
      cookie = oneLine`
        koss:test_sid=bGX1r5jcQHX4CqO1Heiy_DLvkTpQvx3M; path=/session;
        expires=Thu, 13 Apr 2017 17:19:04 GMT; httponly;
        koss:test_sid.sig=ZvZ8W0x9akbySx-9kEUkVPqAd2g; path=/session;
        expires=Thu, 13 Apr 2017 17:19:04 GMT; httponly
      `
      request(commonApp)
        .get('/session/get')
