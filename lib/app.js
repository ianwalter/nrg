...
module.exports = function createApp (options = {}) {
  ...
  // user-supplied.
  app.isProduction = options.isProduction = options.env === 'production'
  ...
  // environment.
  options.log.level = options.log.level || (app.isProduction ? 'info' : 'debug')
  options.log.redact = !options.log.redact && app.isProduction
  ...
  // TODO: comment
  if (options.static) {
    const isObject = typeof options.static === 'object'
    app.use(serveStatic(isObject ? options.static : {}))
  }
  ...
  // production.
  app.use(json({ pretty: !app.isProduction }))
  ...
  // TODO:
  if (options.ssr) {
    const isObject = typeof options.ssr === 'object'
    app.use(serveWebpack(isObject ? options.ssr : {}))
  }
