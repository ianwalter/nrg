...
module.exports = function createApp (options = {}) {
  ...
  app.start = function start () {
    ...
    return new Promise(resolve => {
      app.server.listen(app.context.cfg.port, app.context.cfg.host, err => {
        if (err) {
        ...
        app.server.url = app.context.cfg.hostUrl

        // NOTE: pino is not including the URL in the message if passed as the
        // second param here.
        app.logger.info(oneLine`
          ${app.context.cfg.name} now serving requests at: ${app.server.url}
        `)
