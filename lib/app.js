...
module.exports = function createApp (options = {}) {
  ...
  // environment.
  if (options.log) {
    options.log.level = hasPassedLogLevel || app.isNotDev
      ? options.log.level
      : 'debug'
  ...
  // runtime environment.
  const hasLogConfig = process.env.NODE_ENV !== 'test' && options.log
  const usePino = !options.logger && (hasPassedLogLevel || hasLogConfig)
  app.logger = usePino
    ? pino(options.log)
  ...
  if (options.log.unhandled && usePino) {
