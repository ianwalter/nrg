...
module.exports = function createApp (options = {}) {
  ...
  if (options.mq.enabled || options.mq.connection) {
    const amqplib = require('amqplib')
    app.setup.push(async () => {
      ...
      app.mq = app.context.mq = {
        client,
        channel,
        publish ({ queue, exchange, routingKey }, message, options) {
          return new Promise((resolve, reject) => {
            // Helper method to convert the workflow from callback to promise.
            function handlePublish (err, ok) {
              if (err) return reject(err)
              resolve(ok)
            }

            // Convert the message to JSON string and convert that to a Buffer.
            message = Buffer.from(JSON.stringify(message))

            // Publish the message to the queue or exchange.
            if (queue) {
              channel.sendToQueue(queue, message, handlePublish)
            } else {
              channel.publish(exchange, routingKey, message, handlePublish)
            }
          })
        },
        consume (queue, fn, options) {
          fn = $raw => fn({ $raw, ...JSON.parse($raw.content.toString()) })
          channel.consume(queue, fn, options)
        }
      }
        ...
        await Promise.all(options.mq.queues.map(async queue => {
          ...
          app.mq[queue] = {
            publish (message, options) {
              return app.mq.publish(queue, message, options)
            },
            consume (fn, options) {
              app.mq.consume(queue, fn, options)
            }
          }
        }))
      }
  ...
  // connections.
  app.start = async function start (waitForSetup) {
    ...
    // Wait for asynchronous setup to complete if necessary.
    if (waitForSetup && app.setup.length) {
      return Promise.all(app.setup)
    }
  }
