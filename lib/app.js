...
// const defaults
// function generateCompleteDefaults
// options = merge({}, generateCompleteDefaults(options), options)

function generateDefaults (options) {
  const defaultBaseUrl = `http://localhost:${options.port || 9999}`
  return {
    name: packageJson.name || 'nrg app',
    baseUrl: packageJson.homepage || defaultBaseUrl,
    ...
    log: {
      ...
      unhandled: true,
      prettyPrint: isProd ? false : { stream: false },
      prettifier: isProd ? false : require('@ianwalter/pino-print'),
...
module.exports = function createApp (options = {}) {
  ...
  // Add an array for asynchronous setup tasks so they can be waited on if
  // necessary.
  app.asyncSetupTasks = []
  ...
  // TODO: comment
  app.addAsyncSetupTask = function addAsyncSetup (asyncFn) {
    app.asyncSetupTasks.push(asyncFn())
  }

  // Set up the message queue client if enabled.
  if (options.mq.enabled || options.mq.connection || options.mq.queues) {
    const amqplib = require('amqplib')
    app.addAsyncSetupTask(async () => {
      // Connect to the AMQP server and create a channel.
      const client = await amqplib.connect(options.mq.connection)
      const channel = await client.createConfirmChannel()

      app.mq = app.context.mq = {
        client,
        channel,
        pub ({ exchange = '', routingKey, queue }, content, options) {
        ...
        sub (queue, fn, options) {
          const handler = rawMessage => {
            const message = clone(rawMessage)
            message.ack = () => channel.ack(rawMessage)
            message.content = JSON.parse(rawMessage.content.toString())
            app.logger.debug(message, `Received message on queue ${queue}`)
            fn(message)
          }
      ...
      // Add a wrapper with a nicer API to the app's mq object for each queue.
      if (options.mq.queues) {
        await Promise.all(options.mq.queues.map(async queue => {
          // I don't know why it makes you do this.
          await app.mq.channel.assertQueue(queue)

          // A nicer API that uses JSON-compatible objects as messages instead
          // of Buffers.
          app.mq[queue] = {
            pub (content, options) {
              return app.mq.pub({ queue }, content, options)
            },
            sub (fn, options) {
              app.mq.sub(queue, fn, options)
            }
          }
        }))
      }
  ...
  if (options.accounts.enabled) {
    ...
    app.context.renderEmail = function renderEmail (key, data) {
      const renderer = options.accounts.emails[key]
      if (renderer) {
        return renderer(data)
      }
      app.logger.error(`Email renderer for ${key} not found`)
    }
  }
  ...
  // TODO: comment
  app.asyncSetup = Promise.all(app.asyncSetupTasks).catch(err => {
  ...
  app.start = function start () {
    ...
    app.logger.info('nrg now serving requests at:', `http://${host}:${port}`)
    app.server = server
  }
  ...
  app.test = function test (url, options = {}) {
    ...
    // If options is a re-used response object, just extract and use the cookie.
    if (options.status) {
      // Supertest does not like a undefined Cookie.
      const Cookie = options.headers['set-cookie']
    ...
    return {
      ...
      run (request) {
        // Wait for asynchronous setup to complete before making the request.
        return new Promise(resolve => app.asyncSetup.then(() => {
          request.end((_, res) => resolve(res))
        }))
      }
    }
  }
