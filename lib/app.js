...
function generateDefaults (options) {
  ...
  return {
    ...
    log: {
      ...
      redact: options.log && options.log.level !== 'debug'
...
module.exports = function createApp (options = {}) {
  ...
  // runtime environment.
  const usePino = options.log && !nrgCli
  const logger = app.logger = usePino
    ? pino(options.log)
    : options.logger || new Print({ level: 'error' })
  ...
  if (!nrgCli) {
    app.use(options.handleError)
  }

  // Add middleware that redirects from http to https when in production or
  // otherwise specified.
  if (isProd && options.middleware.httpsRedirect) {
    app.use(options.middleware.httpsRedirect)
  }
  ...
  if (!nrgCli && options.static.enabled) {
    ...
    } else if (options.static.webpack.enabled) {
      // TODO:
  ...
  if (!nrgCli) {
    ...
    // etc.
    if (options.middleware.bodyParser) {
      app.use(options.middleware.bodyParser)
    }
    ...
    return new Promise((resolve, reject) => {
      app.server.listen(port, host, err => {
        ...
        app.logger.info(
          ...
          // second param here.
          `${options.name} now serving requests at: ${app.server.url}`
        )
