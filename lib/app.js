...
const { SchemaValidator, isEmail } = require('@ianwalter/correct')
...
const defaults = {
  ...
  email: {
    transport: { pool: isNotDev, ignoreTLS: !isNotDev },
    mailgen: {
      product: {
        name: packageJson.name,
...
module.exports = function createApp (options = {}) {
  ...
  if (options.url) {
    defaults.email.mailgen.link = options.url
  }
  ...
  if (options.accounts.enabled) {
    ...
    app.context.validators = {
      ...
      email: new SchemaValidator({ email: { isEmail } })
    }
