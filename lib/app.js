...
const defaults = {
  env: process.env.NRG_ENV || process.env.NODE_ENV,
  log: {
    level: 'info',
    unhandled: true
  },
...
module.exports = function createApp (options = {}) {
  ...
  // Create the isProduction variable so sane default configuration can be used
  // when running in a production environment without having to be
  // user-supplied.
  app.context.isProduction = options.isProduction = options.env === 'production'
  ...
  // environment.
  options.log.level = options.log.level ||
    (app.context.isProduction ? 'info' : 'debug')
  options.log.redact = !options.log.redact && app.context.isProduction
  ...
  if (options.static.enabled) {
    if (app.context.isProduction || !options.static.webpack.enabled) {
      //
  ...
  // production.
  app.use(json({ pretty: !app.context.isProduction }))
  ...
  app.start = function start (address) {
    ...
    logger.info('nrg now serving requests at:', url.href)
    app.server = server
  }
