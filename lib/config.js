...
module.exports = function config (options = {}) {
  const cfg = {
    ...
    plugins: {
      ...
      // production mode.
      httpsRedirect (app) {
        if (cfg.isProd) app.use(httpsRedirect)
      },
      // Middleware for serving static files using koa-send. Not enabled by
      // default.
      static (app) {
        const enabled = options.static || this.webpack?.enabled
        if (enabled) app.use(serveStatic(cfg.static))
      },
      // FIXME: comment.
      webpack (app) {
        if (options.webpack) app.use(serveWebpack(cfg.webpack))
      },
      ...
      // enabled.
      csrf (app) {
        if (options.keys?.length && this.session) {
      ...
      health (app) {
        if (this.router) app.get('/health', ctx => (ctx.status = 200))
      },
