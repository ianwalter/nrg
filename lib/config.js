...
module.exports = function config (options = {}) {
  const cfg = {
    ...
    log: {
      ...
      // to a configuration object that tells the default prettifier,
      // pino-print, to not verbosely log requests for static files with paths
      // that include '/static/' or the specified `static` value.
      get prettyPrint () {
        return cfg.isProd ? false : { static: '/static/', level: this.level }
      },
      // [Object] A pino-compatible prettifier. Defaults to @
      ...
      // [Function] A function that tells pino what the request ID should be
      // given a request object. Defaults to req.id.
      genReqId: req => req.id
    },
    ...
    middleware: {
      ...
      get session () {
        if (options.keys?.length) {
          ...
          return app => session({ store: app.redis }, app)
        }
        return false
      },
