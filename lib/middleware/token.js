...
async function generateToken (ctx, next) {
  ctx.result = { token: uid.sync(ctx.options.hash.bytes) }
  ctx.result.hashedToken = await hash(ctx.result.token, ctx.options.hash.rounds)
  next()
}
...
async function insertTokenFromCtx (ctx, tokenType) {
  ...
  if (tokenType === 'email' && account && account.emailVerified) {
    ...
    ctx.log.debug({ ...ctx.data, tokenType, ...ctx.result }, 'Inserting token')
