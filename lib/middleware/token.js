...
async function handleGenerateToken (ctx, next, options) {
  ctx.result = { token: await uid(options.bytes || ctx.options.hash.bytes) }
...
async function handleInsertToken (ctx, next, options) {
  ...
  const account = ctx.session.account ||
    await Account.query().findOne({ email: ctx.data.email })
  ...
  if (!ctx.result.body.message) {
    const t = options.type === 'email'
      ? 'Email Verification'
      : 'Forgot Password'
    ctx.result.body.message = `${t} request submitted successfully`
  }

  if (options.type === 'email' && !emailChanged && account.emailVerified) {
