...
async function handleInsertToken (ctx, next, options) {
  ...
  // Add a emailChanged property to the state so that accountUpdate can know to
  // reset emailVerified to false.
  ctx.state.emailChanged = account && account.email !== payload.email
  ...
  if (isEmail && !ctx.state.emailChanged && account && account.emailVerified) {
    ...
    ctx.log.debug({ ...payload, ...options, ...ctx.state }, 'Inserting token')
...
function verifyToken (property = 'passwordTokens') {
  return async (ctx, next) => {
    ...
    const data = { hasToken, token, ...payload, tokensMatch }
