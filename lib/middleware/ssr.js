...
function serveSsr (options) {
  ...
  // Create the page renderer that will be used by the middleware if it wasn't
  ...
  if (!options.renderer) {
    try {
      ...
      options.renderer = require(options.entry)(options.page)
    } catch (err) {
      print.warn(err)
    }
  }
  ...
  return (ctx, next) => {
    // If not in production mode, use the renderer generated by Webpack so that
    // it can be changed without having to restart the server.
    if (!ctx.isProduction) {
