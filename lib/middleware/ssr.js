...
function serveSsr (options) {
  ...
  return async (ctx, next) => {
    if (ctx.isProduction && !options.renderer) {
      ...
      const entry = ctx.webpack.fileSystem.readFileSync(options.entry, 'utf8')
      const page = ctx.webpack.fileSystem.readFileSync(options.template, 'utf8')
      options.renderer = requireFromString(entry).default(page)
    }
