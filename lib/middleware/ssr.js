...
function serveSsr (options) {
  ...
  return async (ctx, next) => {
    ...
    if (typeof result === 'string') {
      ctx.log.debug('Redirecting to:', result)
    ...
    if (!ctx.body) {
      ...
      // If result is falsy, e.g. a 404 page isn't even returned, continue to
      // the next middleware instead of returning a blank page.
      if (!html) {
        return next()
      }
