...
/**
 * Query the database for the account that is attempting to login.
 */
async function getLoginAccount (ctx, next) {
  const Account = ctx.options.accounts.models.account
  ctx.account = await Account.query().findOne({ email: ctx.data.email })
  next()
}
 ...
 * otherwiser return a 401 Unauthorized response.
 */
