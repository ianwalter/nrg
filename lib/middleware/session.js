...
async function validateLogin (ctx, next) {
  ...
  ctx.log.debug(validation, 'validateLogin')
...
async function createUserSession (ctx, next) {
  ctx.log.debug(ctx.state.account || {}, 'createUserSession')

  if (ctx.state.account?.enabled) {
    ctx.log.info(oneLine`
      Successfully created session for account: ${ctx.state.account.id}
    `)
